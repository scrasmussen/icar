...
files_tmp3=$(filter-out src_tests_%,$(files_tmp2))
files=$(addprefix build/caf_*/icar/,$(files_tmp3))

.PHONY: print build
all: $(files) build
	@echo "Compiled by fpm"

build:
	bash scripts/build-icar.sh

# from foo/file.f90 the fpm creates build/caf_*/icar/foo_file.f90.o
# every file is listed since some patterns weren't matching when they should
